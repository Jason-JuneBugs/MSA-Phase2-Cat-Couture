...
namespace Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task UpdateProductAsync(Product product)
        {
            ...
            // Update related ProductCategory (if exists)
            if (product.ProductCategory != null)
            {
                _context.Entry(product.ProductCategory).State = EntityState.Modified;
            }

            // Update related ProductImage (if exists)
            if (product.ProductImage != null)
            {
                _context.Entry(product.ProductImage).State = EntityState.Modified;
            }
        ...
        public async Task DeleteProductAsync(int id)
        {
            ...
            if (product != null)
            {
                // Remove related ProductCategory (if exists)
                if (product.ProductCategory != null)
                {
                    _context.ProductCategories.Remove(product.ProductCategory);
                }

                // Remove related ProductImage (if exists)
                if (product.ProductImage != null)
                {
